...
import { AnimatePresence } from 'motion/react';
import { DEFAULT_CONFIG } from './data/defaultData';
import { AppConfig, SceneType, PhotoItem } from './types';
import { musicPlayer } from './utils/audio';
...
// Minimum distance (in % units) do photos ke beech honi chahiye taki overlap na ho
const MIN_DISTANCE = 22;

function getNonOverlappingPosition(existingPhotos: PhotoItem[]): { x: number; y: number } {
  ...
  // 30 attempts ke baad bhi jagah na mile, toh grid-fallback position do
  ...
  const handleAddGalleryPhotos = (files: FileList) => {
    (Array.from(files) as File[]).forEach((file: File, index: number) => {
      ...
      reader.onload = (ev) => {
        if (ev.target?.result) {
          setConfig((prev) => {
            const { x, y } = getNonOverlappingPosition(prev.galleryPhotos);
            const newPhoto = {
              id: `upload-${Date.now()}-${index}`,
              url: ev.target!.result as string,
              caption: file.name.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ') || 'Cherished Memory ❤️',
...
// // Existing photos check karke overlapping wali ko reposition karta hai
// function deduplicateOverlappingPhotos(photos: PhotoItem[]): { photos: PhotoItem[]; changed: boolean } {
//   const fixed: PhotoItem[] = [];
//   let changed = false;
...
//     const overlapsExisting = fixed.some((f) => {
//       const fx = f.x ?? 50;
//       const fy = f.y ?? 50;
//       const distance = Math.sqrt((px - fx) ** 2 + (py - fy) ** 2);
//       return distance < MIN_DISTANCE;
//     });

//     if (overlapsExisting) {
//       const { x, y } = getNonOverlappingPosition(fixed);
//       fixed.push({ ...photo, x, y });
//       changed = true;
//     } else {
//       fixed.push(photo);
//     }
...
//   // One-time migration: purani overlapping photos ko fix karo
//   useEffect(() => {
//     setConfig((prev) => {
//       const { photos, changed } = deduplicateOverlappingPhotos(prev.galleryPhotos);
//       if (!changed) return prev;

//       const updated = { ...prev, galleryPhotos: photos };
//       try {
//         localStorage.setItem(STORAGE_KEY, JSON.stringify(updated));
//       } catch (e) {}
//       return updated;
